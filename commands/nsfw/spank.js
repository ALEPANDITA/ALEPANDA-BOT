...
module.exports = {
  name: 'spank',
  category: 'nsfw',
  description: 'Le da una nalgada a alguien (menciona o responde)',
  execute: async (sock, jid, msg) => {
    const tempGif = path.join(__dirname, `temp_spank_${Date.now()}.gif`);
    const tempMp4 = path.join(__dirname, `temp_spank_${Date.now()}.mp4`);
    ...
    try {
      ...
      let gifUrl = '';
      try {
        const data = await getJson('https://purrbot.site/api/img/nsfw/spank/gif');
        if (data && data.link) gifUrl = data.link;
      } catch (err) {}

      if (!gifUrl) {
        try {
          const data = await getJson('https://nekos.life/api/v2/img/spank');
      ...
      let captionText = (mentioned && mentioned !== sender)
        ? `👋💥 @${sender.split('@')[0]} le acomodó una sonora nalgada a @${mentioned.split('@')[0]} 😈`
        : `👋💥 @${sender.split('@')[0]} se anda dando nalgadas solo/a 😳`;
      ...
      console.error('Error en spank:', error);
