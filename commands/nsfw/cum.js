...
module.exports = {
  name: 'cum',
  category: 'nsfw',
  description: 'Se viene sobre alguien (menciona o responde)',
  execute: async (sock, jid, msg) => {
    const tempGif = path.join(__dirname, `temp_cum_${Date.now()}.gif`);
    const tempMp4 = path.join(__dirname, `temp_cum_${Date.now()}.mp4`);
    ...
    try {
      ...
      let gifUrl = '';
      try {
        const data = await getJson('https://purrbot.site/api/img/nsfw/cum/gif');
        if (data && data.link) gifUrl = data.link;
      } catch (err) {}

      if (!gifUrl) {
        try {
          const data = await getJson('https://nekos.life/api/v2/img/cum');
      ...
      let captionText = (mentioned && mentioned !== sender)
        ? `💦 @${sender.split('@')[0]} se vino sobre @${mentioned.split('@')[0]} 😈`
        : `💦 @${sender.split('@')[0]} se vino solo 💦`;
      ...
      console.error('Error en cum:', error);
