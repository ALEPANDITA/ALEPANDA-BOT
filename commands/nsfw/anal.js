...
module.exports = {
  name: 'anal',
  category: 'nsfw',
  description: 'Le hace un anal a alguien (menciona o responde)',
  execute: async (sock, jid, msg) => {
    const tempGif = path.join(__dirname, `temp_anal_${Date.now()}.gif`);
    const tempMp4 = path.join(__dirname, `temp_anal_${Date.now()}.mp4`);
    ...
    try {
      ...
      let gifUrl = '';
      try {
        const data = await getJson('https://purrbot.site/api/img/nsfw/anal/gif');
      ...
      let captionText = (mentioned && mentioned !== sender)
        ? `🍑 @${sender.split('@')[0]} le está dando por el culo a @${mentioned.split('@')[0]} 🔥`
        : `🍑 @${sender.split('@')[0]} anda muy goloso/a 😈`;
      ...
      console.error('Error en anal:', error);
