...
module.exports = {
  name: 'pussylick',
  category: 'nsfw',
  description: 'Lame el coño de alguien (menciona o responde)',
  execute: async (sock, jid, msg) => {
    ...
    try {
      ...
      let gifUrl = '';
      try {
        const data = await getJson('https://purrbot.site/api/img/nsfw/pussylick/gif');
        if (data && data.link) gifUrl = data.link;
      } catch (err) {}

      if (!gifUrl) {
        try {
          const data = await getJson('https://nekos.life/api/v2/img/pussy');
      ...
      let captionText = (mentioned && mentioned !== sender)
        ? `👅 @${sender.split('@')[0]} le está lamiendo todo el coño a @${mentioned.split('@')[0]} 🤤`
        : `👅 @${sender.split('@')[0]} anda buscando a quién lamer 😈`;
      ...
      console.error('Error en pussylick:', error);
