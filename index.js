...
// Emoji de reaccion segun la categoria del comando
...
function mostrarBanner() {
  ...
  const colorSoportado = chalk.level > 0;

  // Si la terminal es muy angosta (paneles web en celular) o no soporta colores,
  // usamos un banner simple de texto en vez del arte ASCII, para que nunca se vea roto.
  if (columnas < 45 || !colorSoportado) {
    console.log('=========================');
    console.log('      ALEPANDA BOT');
    console.log('  Edicion Hogwarts');
    console.log('=========================');
    console.log('made with love by ALEPANDITA');
    console.log('');
    return;
  }

  const texto = figlet.textSync('ALEPANDA', { font: 'Small' });

  try {
    console.log(gradient.pastel.multiline(texto));
    console.log(gradient.cristal('⚡ made with love by ALEPANDITA ⚡'));
  } catch {
    // Si gradient-string falla por cualquier razon, no se rompe el arranque del bot
    console.log(chalk.cyanBright(texto));
    console.log(chalk.magentaBright('⚡ made with love by ALEPANDITA ⚡'));
  }
...
async function startBot() {
    ...
    if (metodo?.trim() === '1') {
      ...
      for (let intento = 1; intento <= 5 && !codigo; intento++) {
        try {
          ...
          ultimoError = err;
          console.log(`Intento ${intento} fallo (${err?.message || err}), reintentando...`);
        }
      }

      if (!codigo) {
        console.log('No se pudo obtener el codigo de vinculacion despues de varios intentos.');
        console.log('Revisa tu conexion a internet, o intenta con la opcion 1 (codigo QR).');
        console.log('Ultimo error:', ultimoError?.message || ultimoError);
        process.exit(1);
      }
  ...
  sock.ev.on('group-participants.update', async (update) => {
    ...
    for (const participante of participants) {
      ...
      if (action === 'add' && grupo.antiraid) {
        ...
        if (reciente.length > 5) {
          try {
            ...
            await sock.sendMessage(jid, {
              text: '🚨 Posible raid detectado (muchos usuarios entrando de golpe). El grupo fue cerrado automaticamente, solo admins pueden escribir.'
            });
          } catch (err) {
            console.error(err);
          }
      ...
      if (action === 'add' && grupo.bienvenida) {
        const { construirTexto, construirPayloadEnvio, obtenerMediaGuardada } = require('./lib/bienvenidapro');
        ...
        const { buffer, tipoMedia } = obtenerMediaGuardada('welcome', jid, grupo);
        const payload = construirPayloadEnvio(tipoMedia, buffer, texto);
        await sock.sendMessage(jid, { ...payload, mentions: [participanteId] });
      }
      ...
      if (action === 'remove' && grupo.despedida) {
        ...
        const texto = construirTexto(plantilla, { numero, metadata, sock, prefix: config.prefix });
        const { buffer, tipoMedia } = obtenerMediaGuardada('bye', jid, grupo);
        const payload = construirPayloadEnvio(tipoMedia, buffer, texto);
        await sock.sendMessage(jid, { ...payload, mentions: [participanteId] });
      }
    }
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    ...
    // Distintos clientes de WhatsApp (Android, iPhone, WhatsApp Web, versiones viejas)
    // pueden devolver la seleccion de un boton/lista en formatos distintos.
    // Revisamos todos los formatos conocidos para que funcione en cualquier dispositivo.
    ...
    if (!idSeleccionado) {
      idSeleccionado = msg.message?.listResponseMessage?.singleSelectReply?.selectedRowId || null;
    }

    if (!idSeleccionado) {
      idSeleccionado = msg.message?.buttonsResponseMessage?.selectedButtonId || null;
    }
