...
const REGEX_INVITA_SIMI = /\b(tra\w*|invit\w*|met\w*|llam\w*|dile|dila|dilo|habl\w*)\b.*\bsimi\b/i;
const REGEX_INVITA_PANDA = /\b(tra\w*|invit\w*|met\w*|llam\w*|dile|dila|dilo|habl\w*)\b.*\bpanda\b/i;
const REGEX_INVITA_SIMON = /\b(tra\w*|invit\w*|met\w*|llam\w*|dile|dila|dilo|habl\w*)\b.*\bsimon\b/i;
const REGEX_INVITA_CUPIDO = /\b(tra\w*|invit\w*|met\w*|llam\w*|dile|dila|dilo|habl\w*)\b.*\bcupido\b/i;
...
// --- Helper de menciones (@persona / @todos), compartido entre Simi, Panda, Simon y Cupido ---
    ...
    try {
      ...
      numerosVisibles = mencionados.map(id => {
        ...
        const participante = metadata.participants.find(p =>
          (p.id || '').split('@')[0] === miId || (p.lid || '').split('@')[0] === miId
        );
        const numeroReal = (participante?.phoneNumber || '').split('@')[0];
        return numeroReal || miId;
      });
    } catch (err) {
...
function armarMensajeParaIA(mensaje, { esTodos, etiquetasTags }) {
  if (esTodos) {
    return `(Estas hablando con todo el grupo, dirigete a ellos como "ustedes" o "todos") ${mensaje}`;
  }
...
function armarMensajeParaRespuestaDirecta({ nombreOtroBot, mensajeUsuario, respuestaOtroBot }) {
  return `(${nombreOtroBot} te acaba de contestar directamente a TI dentro de esta conversacion de WhatsApp, no solo a la persona. La persona habia escrito: "${mensajeUsuario}". ${nombreOtroBot} te dijo: "${respuestaOtroBot}". Ahora contestale tu directamente a ${nombreOtroBot}, con tu tono, como si le respondieras a el y no a la persona. No repitas lo que ya dijiste antes, reacciona especificamente a lo que ${nombreOtroBot} te acaba de decir a ti.) ${mensajeUsuario}`;
}

// Helper compartido por simi/panda/simon/cupido.js: mete al amigo invitado a la
// conversacion (le responde a la persona) y luego el anfitrion le responde de
// vuelta al invitado, para que se sienta como una conversacion real entre los bots.
async function invitarYResponder(sock, jid, msg, {
  ...
  try {
    ...
    const textoInvitado = armarTextoFinal(respuestaInvitado, { esTodos, etiquetasTags });

    await sock.sendMessage(jid, { text: `${emojiInvitado} *${nombreInvitado}:* ${textoInvitado}`, mentions: mencionados }, { quoted: msg });
    ...
    await sock.sendMessage(jid, { text: `${emojiAnfitrion} *${nombreAnfitrion}:* ${respuestaDeVuelta}`, mentions: mencionados }, { quoted: msg });
  } catch (err) {
    // Si falla el rebote entre bots, no tumbamos la respuesta principal que ya se mando:
    // solo lo dejamos en el log.
    console.error(`Error invitando a ${nombreInvitado} a la conversacion:`, err.message || err);
  }
}
