...
async function obtenerReaccionDelirius(endpoint) {
  const data = await fetchJsonSeguro(`https://api.delirius.store/reactions/${endpoint}`);
  return { url: data.data.url };
}

async function enviarReaccionDelirius(sock, jid, msg, { endpoint, emoji, conObjetivo, sinObjetivo }) {
  const mencionado = msg.message?.extendedTextMessage?.contextInfo?.mentionedJid?.[0];
  const citado = msg.message?.extendedTextMessage?.contextInfo?.participant;
  const remitente = msg.key.participant || msg.key.remoteJid;
  const objetivo = mencionado || citado;
  ...
  try {
    const reaccion = await obtenerReaccionDelirius(endpoint);
...
module.exports = { obtenerReaccion, enviarReaccion, obtenerReaccionDelirius, enviarReaccionDelirius };
