...
const { generarTextoGroq, chatConPersonalidadGroq } = require('./groq');
const { generarImagenPollinations } = require('./imagenGratis');
...
// Si Gemini no tiene ninguna clave o todas se quedaron sin cupo, vale la pena
// intentar con Groq (otro servicio gratis) antes de darnos por vencidos.
function puedeCaerAGroq(err) {
  return Boolean(err && (err.code === 'NO_API_KEY' || err.todasLasClavesFallaron));
}

function errorCombinado(errGemini, errGroq) {
  return new Error(`Gemini no respondio (${errGemini.message}) y Groq tampoco (${errGroq.message}).`);
}
...
async function generarTexto(prompt, { imagenBase64, mimeType } = {}) {
  ...
  try {
    return await conReintentos({ contents: [{ parts }] });
  } catch (err) {
    if (!puedeCaerAGroq(err) || imagenBase64) throw err; // Groq no puede ver imagenes
    try {
      return await generarTextoGroq(prompt);
    } catch (errGroq) {
      throw errorCombinado(err, errGroq);
    }
  }
}
...
async function chatConPersonalidad(systemInstruction, historial, mensajeNuevo) {
  ...
  try {
    ...
    if (!puedeCaerAGroq(err)) throw err;
    try {
      return await chatConPersonalidadGroq(systemInstruction, historial, mensajeNuevo);
    } catch (errGroq) {
      throw errorCombinado(err, errGroq);
    }
  }
...
async function llamarModeloImagen(modelo, prompt, apiKey, imagenEntrada) {
  const url = `https://generativelanguage.googleapis.com/v1beta/models/${modelo}:generateContent?key=${apiKey}`;

  const parts = [];
  if (imagenEntrada) {
    parts.push({ inline_data: { mime_type: imagenEntrada.mimeType || 'image/jpeg', data: imagenEntrada.base64 } });
  }
  parts.push({ text: prompt });
...
// imagenEntrada es opcional: { base64, mimeType }. Si viene, Gemini EDITA esa
// imagen en vez de crear una desde cero (funciona con gemini-2.5-flash-image).
async function generarImagen(prompt, imagenEntrada) {
  try {
    return await conRotacionDeClaves(async (apiKey) => {
      let ultimoError;
      for (const modelo of MODELOS_IMAGEN) {
        try {
          return await llamarModeloImagen(modelo, prompt, apiKey, imagenEntrada);
        } catch (err) {
          ultimoError = err;
    ...
    if (!puedeCaerAGroq(err)) throw err; // reutilizamos el mismo criterio: sin clave o todas agotadas
    if (imagenEntrada) throw err; // Pollinations no puede editar una imagen que le mandes, solo crear desde cero
    try {
      return await generarImagenPollinations(prompt);
    } catch (errPollinations) {
      throw new Error(`Gemini no pudo generar la imagen (${err.message}) y Pollinations tampoco (${errPollinations.message}).`);
    }
  }
}

// Extrae el "Please retry in 49.88s" que manda Google, si viene en el mensaje
function extraerEsperaSegundos(mensaje = '') {
  const match = mensaje.match(/retry in ([\d.]+)s/i);
  return match ? Math.ceil(Number(match[1])) : null;
}

// Prueba UNA clave especifica contra el modelo real que usa el bot, sin rotar
// a otra clave. Sirve para que el owner vea el estado real de cada clave.
async function probarClave(apiKey) {
  const modelo = MODELOS_CHAT[0];
  const inicio = Date.now();

  try {
    await llamarModelo(modelo, { contents: [{ parts: [{ text: 'hola' }] }] }, apiKey);
    return { ok: true, ms: Date.now() - inicio, modelo };
  } catch (err) {
    const espera = extraerEsperaSegundos(err.message);
    return {
      ok: false,
      modelo,
      esperaSegundos: espera,
      esCuota: err.errorDeClave || false,
      esTemporal: err.temporal || false,
      mensaje: err.message
    };
  }
}

module.exports = { generarTexto, generarImagen, chatConPersonalidad, probarClave };
